...
def getSorts(lvl):
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche Magique", 1, 3, 1, 8, [EffetDegats(13, 15, "Air"), EffetEtat(EtatBoostCaracFixe("Fleche Magique", 0, 1, "PO", -2)), EffetEtatSelf(EtatBoostCaracFixe("Fleche Magique", 0, 1, "PO", 2))], [EffetDegats(16, 18, "Air"), EffetEtat(
            EtatBoostCaracFixe("Fleche Magique", 0, 1, "PO", -2)), EffetEtatSelf(EtatBoostCaracFixe("Fleche Magique", 0, 1, "PO", 2))], 15, 3, 2, 0, 1, "cercle", True, description="""Occasionne des dommages Air et vole la port�e de la cible.""", chaine=True),
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche érosive", 105, 3, 1, 3, [EffetEtat(EtatBoostCaracFixe("Fleche Erosive", 0, 2, "erosion", 10)), EffetDegats(25, 29, "Terre")], [EffetEtat(EtatBoostCaracFixe(
            "Fleche Erosive", 0, 2, "erosion", 10)), EffetDegats(29, 33, "Terre")], 15, 3, 2, 0, 1, "ligne", True, description="""Occasionne des dommages Terre et applique un malus d'�rosion.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche Répulsive", 120, 3, 1, 7, [EffetDegats(28, 32, "Feu", cible_non_requise=True, zone=Zones.TypeZoneLignePerpendiculaire(1)), EffetPousser(1, "CaseCible", cible_non_requise=True, zone=Zones.TypeZoneLignePerpendiculaire(1))], [EffetDegats(
            34, 38, "Feu", zone=Zones.TypeZoneLignePerpendiculaire(1), cible_non_requise=True), EffetPousser(1, "CibleDirect", cible_non_requise=True, zone=Zones.TypeZoneLignePerpendiculaire(1))], 5, 2, 99, 0, 0, "ligne", True, description="""Occasionne des dommages Feu et repousse de 1 case.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Tir éloigné", 9, 3, 0, 0, [EffetEtat(EtatBoostCaracFixe("Tir_eloigne", 0, 4, "PO", 2), zone=Zones.TypeZoneCercle(2))], [EffetEtat(EtatBoostCaracFixe(
            "Tir_eloigne", 0, 4, "PO", 3), zone=Zones.TypeZoneCercle(2))], 25, 1, 1, 5, 0, "cercle", False, description="""Augmente la port�e des cibles pr�sentes dans la zone d'effet.""", chaine=True),
        ...
        Sort.Sort("Tir éloigné", 47, 3, 0, 0, [EffetEtat(EtatBoostCaracFixe("Tir_eloigne", 0, 4, "PO", 4), zone=Zones.TypeZoneCercle(2))], [EffetEtat(EtatBoostCaracFixe(
            "Tir_eloigne", 0, 4, "PO", 5), zone=Zones.TypeZoneCercle(2))], 25, 1, 1, 5, 0, "cercle", False, description="""Augmente la port�e des cibles pr�sentes dans la zone d'effet.""", chaine=True),
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche de Rédemption", 130, 3, 6, 8, [EffetDegats(19, 22, "Eau"), EffetEtatSelf(EtatBoostBaseDeg("Flèche de rédemption", 1, 1, "Flèche de Rédemption", 12))], [EffetDegats(23, 26, "Eau"), EffetEtatSelf(
            EtatBoostBaseDeg("Flèche de rédemption", 1, 1, "Flèche de Rédemption", 12))], 25, 3, 2, 0, 1, "cercle", True, description="""Occasionne des dommages Eau qui sont augment�s si le sort est relanc� le tour suivant.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche écrasante", 135, 3, 5, 7, [EffetEtat(Etat("Pesanteur", 0, 1), cible_non_requise=True, zone=Zones.TypeZoneCroixDiagonale(1)), EffetDegats(34, 38, "Feu", cible_non_requise=True, zone=Zones.TypeZoneCroixDiagonale(1))], [EffetEtat(Etat(
            "Pesanteur", 1, 1), cible_non_requise=True, zone=Zones.TypeZoneCroixDiagonale(1)), EffetDegats(37, 41, "Feu", cible_non_requise=True, zone=Zones.TypeZoneCroixDiagonale(1))], 25, 1, 1, 3, 1, "cercle", True, description="""Occasionne des dommages Feu et applique l'�tat Pesanteur.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Tir Critique", 22, 2, 0, 2, [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 10))], [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 10)), EffetEtat(
            EtatBoostCaracFixe("Tir Critique Critique", 0, 4, "pui", 10))], 25, 1, 1, 5, 1, "cercle", True, description="""Augmente la probabilit� de faire un coup critique.""", chaine=True),
        ...
        Sort.Sort("Tir Critique", 65, 2, 0, 4, [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 12))], [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 12)), EffetEtat(
            EtatBoostCaracFixe("Tir Critique Critique", 0, 4, "pui", 30))], 25, 1, 1, 5, 1, "cercle", True, description="""Augmente la probabilit� de faire un coup critique.""", chaine=True),
        ...
        Sort.Sort("Tir Critique", 108, 2, 0, 6, [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 14))], [EffetEtat(EtatBoostCaracFixe("Tir Critique", 0, 4, "cc", 14)), EffetEtat(
            EtatBoostCaracFixe("Tir Critique Critique", 0, 4, "pui", 50))], 25, 1, 1, 5, 1, "cercle", True, description="""Augmente la probabilit� de faire un coup critique.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche d'Immobilisation", 27, 2, 1, 6, [EffetDegats(6, 7, "Eau"), EffetEtat(EtatBoostCaracFixe("Flèche d'Immobilisation", 0, 1, "PM", -1)), EffetEtatSelf(EtatBoostCaracFixe("Flèche d'Immobilisation", 0, 1, "PM", 1))], [EffetDegats(7, 8, "Eau"), EffetEtat(
            EtatBoostCaracFixe("Flèche d'Immobilisation", 0, 1, "PM", -1)), EffetEtatSelf(EtatBoostCaracFixe("Flèche d'Immobilisation", 0, 1, "PM", 1))], 5, 4, 2, 0, 1, "cercle", True, description="""Occasionne des dommages Eau et vole des PM � la cible.""", chaine=True),
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche Curative", 165, 3, 3, 6, [EffetSoinPerPVMax(10), EffetEtat(EtatModSoinPer("Flèche Curative", 1, 1, 130))], [EffetSoinPerPVMax(12), EffetEtat(
            EtatModSoinPer("Flèche Curative", 1, 1, 130))], 15, 3, 1, 0, 1, "ligne", True, description="""Soigne et augmente les soins re�us par la cible.""", chaine=True)
    ]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche Dévorante", 180, 3, 1, 6, [
            EffetDegats(70, 74, "Air", zone=Zones.TypeZoneInfini(
            ), etat_requis_cibles="Flèche dévorante lancer 3", consomme_etat=True),
            EffetEtat(Etat("Flèche dévorante lancer 3", 0, -1),
                      etat_requis_cibles="Flèche dévorante lancer 2", consomme_etat=True),
            EffetDegats(52, 56, "Air", zone=Zones.TypeZoneInfini(
            ), etat_requis_cibles="Flèche dévorante lancer 2", consomme_etat=True),
            EffetEtat(Etat("Flèche dévorante lancer 2", 0, -1),
                      etat_requis_cibles="Flèche dévorante lancer 1", consomme_etat=True),
            EffetDegats(34, 38, "Air", zone=Zones.TypeZoneInfini(
            ), etat_requis_cibles="Flèche dévorante lancer 1", consomme_etat=True),
            EffetEtat(Etat("Flèche dévorante lancer 1", 0, -1),
                      etat_requis_cibles="!Flèche dévorante lancer 2|!Flèche dévorante lancer 3")
            ],
                  [
                      EffetDegats(81, 85, "Air", zone=Zones.TypeZoneInfini(),
                                  etat_requis_cibles="Flèche dévorante lancer 3", consomme_etat=True),
                      EffetEtat(Etat("Flèche dévorante lancer 3", 0, -1),
                                etat_requis_cibles="Flèche dévorante lancer 2", consomme_etat=True),
                      EffetDegats(60, 64, "Air", zone=Zones.TypeZoneInfini(),
                                  etat_requis_cibles="Flèche dévorante lancer 2", consomme_etat=True),
                      EffetEtat(Etat("Flèche dévorante lancer 2", 0, -1),
                                etat_requis_cibles="Flèche dévorante lancer 2", consomme_etat=True),
                      EffetDegats(39, 43, "Air", zone=Zones.TypeZoneInfini(),
                                  etat_requis_cibles="Flèche dévorante lancer 1", consomme_etat=True),
                      EffetEtat(Etat("Flèche dévorante lancer 1", 0, -1),
                                etat_requis_cibles="!Flèche dévorante lancer 2|!Flèche dévorante lancer 3")],
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
        Sort.Sort("Flèche Cinglante", 77, 2, 1, 5, [EffetEtat(EtatBoostCaracFixe("Fleche cinglante", 0, 2, "erosion", 10)), EffetPousser(
            2)], [], 0, 4, 2, 0, 1, "ligne", True, description="""Applique de l'�rosion aux ennemis et repousse de 2 cases.""", chaine=True),
    ...
    flecheFulminante = Sort.Sort("Flèche Fulminante", 195, 4, 1, 8, [EffetDegats(38, 42, "Feu", cibles_possibles="Ennemis|Balise Tactique"), EffetEtatSelf(EtatBoostBaseDeg("Flèche Fulminante boost", 0, 1, "Flèche Fulminante Rebond", 10))], [
    ], 0, 1, 1, 0, 1, "cercle", True, description="Occasionne des dommages Feu. Se propage sur l'ennemi le plus proche dans un rayon de 2 cellules. Peut rebondir sur la Balise Tactique. À chaque cible supplémentaire, les dommages du sort sont augmentés.")
    ...
    sorts.append(Personnages.Personnage.getSortRightLvl(
        lvl, [flecheFulminante]))
    sorts.append(Personnages.Personnage.getSortRightLvl(lvl, [
