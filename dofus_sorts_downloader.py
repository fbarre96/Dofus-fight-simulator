"""Download in json format spells from dofus official web site. description are incomplete but it's a start..."""
import requests
...
def main():
    ...
    today = date.today()
    outfolder = today.strftime("%Y-%m-%d")
    os.makedirs(outfolder+"/sorts_icones", exist_ok=True)
    if len(sys.argv) > 1:
        classes = sys.argv[1:]
    else:
    ...
    for classe in classes:
        ...
        with open(outfolder+"/sorts_"+classe+".json","w") as f:
